...
- **Test:** `git grep -c <redacted> HEAD` returns 0. Add to `.github/workflows/path-leak.yml` a full-tree scan (not just PR diff) so this cannot come back.
...
**Phase 1 exit:** 186 bats + 46 pytest green, `git grep <redacted> HEAD` = 0, README renders with demo + badges.
