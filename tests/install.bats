...
@test "install: serena launcher templates strip a caller-supplied start-mcp-server (idempotent dispatch)" {
  # The launchers inject `serena start-mcp-server`, and the host MCP
  # registrations (OpenCode writer, codex TOML, ...) ALSO pass it explicitly.
  # Without a shift guard the subcommand is dispatched twice; the duplicate
  # becomes a positional project arg and serena exits 2 ("--project-from-cwd
  # cannot be used with --project or positional project argument"), which MCP
  # clients surface as error -32000 / connection closed. Both runtime
  # templates (docker + uvx) must carry the guard. In the uvx heredoc the $
  # is escaped (\${1:-}) because that heredoc is unquoted, hence the optional
  # backslash in the pattern.
  run grep -Ec 'if \[ "\\?\$\{1:-\}" = "start-mcp-server" \]; then shift; fi' "$SCRIPTS_DIR/install.sh"
  [ "$status" -eq 0 ]
  [ "${lines[0]}" = "2" ]
}

@test "install: generated docker serena launcher dispatches start-mcp-server exactly once" {
  # Functional companion to the static guard above: generate the real
  # --local launcher against a mocked docker, then invoke it BOTH with and
  # without an explicit start-mcp-server (the two caller dialects) and assert
  # the container command carries the subcommand exactly once either way.
  mock_install_prereqs
  mock_cmd opencode
  echo '{}' > "$TMP_HOME/.opencode.json"
  cat > "$TMP_BIN/docker" << MOCK
#!/usr/bin/env bash
if [ "\$1" = "run" ]; then printf '%s\n' "\$@" > "$TMP_HOME/docker-run-args"; exit 0; fi
exit 0
MOCK
  chmod +x "$TMP_BIN/docker"

  run bash "$SCRIPTS_DIR/install.sh" --serena-only --local --hosts opencode
  [ "$status" -eq 0 ]
  [ -x "$TMP_HOME/.local/bin/serena" ]

  # Caller dialect 1: explicit subcommand (what host MCP configs pass).
  run bash "$TMP_HOME/.local/bin/serena" start-mcp-server --context=ide
  [ "$status" -eq 0 ]
  run grep -cx 'start-mcp-server' "$TMP_HOME/docker-run-args"
  [ "${lines[0]}" = "1" ]
  run grep -qx 'serena' "$TMP_HOME/docker-run-args"
  [ "$status" -eq 0 ]
  rm -f "$TMP_HOME/docker-run-args"

  # Caller dialect 2: no subcommand (bare interactive use).
  run bash "$TMP_HOME/.local/bin/serena" --context=ide
  [ "$status" -eq 0 ]
  run grep -cx 'start-mcp-server' "$TMP_HOME/docker-run-args"
  [ "${lines[0]}" = "1" ]

  # Same --local run must also write the OpenCode entry in the >=1.18 shape.
  python3 - "$TMP_HOME/.opencode.json" << 'PY'
import json, sys
d = json.load(open(sys.argv[1]))
s = (d.get("mcp") or {}).get("serena") or {}
cmd = s.get("command")
assert isinstance(cmd, list), f"command must be an argv array, got: {cmd!r}"
assert "args" not in s, f"OpenCode schema has no args key: {s!r}"
assert s.get("type") == "local", s
assert "start-mcp-server" in cmd, cmd
PY
}

@test "install: opencode serena entry is OpenCode>=1.18 schema shape (command array, no args key)" {
  # OpenCode 1.18 validates mcp entries against a schema whose `command` is a
  # single argv array with no `args` key; the string-command + args shape
  # (regressed in via the SERENA_KEYLOGGER_WRAPPER writer) is silently ignored
  # by the host ("Ignoring MCP config entry without type"). Covers the ONLINE
  # (uvx) writer; the --local writer is covered by the docker launcher test.
  mock_install_prereqs
  mock_cmd opencode
  echo '{}' > "$TMP_HOME/.opencode.json"
...
@test "install: every docker-mode serena registration supplies 'serena start-mcp-server' after the image" {
  # docker/Dockerfile.serena ships intentionally WITHOUT an ENTRYPOINT
  # (v1.11.4 contract: the launcher / registrations are the single source of
  # truth for the serena CLI invocation). A registration that lets the image
  # name be followed directly by --context=... makes docker exec the flag as
  # the container command, which dies instantly. Whitespace is normalised so
  # multi-line python heredoc arrays are checked too. Shell form count is 4:
  # claude-code + gemini real registrations AND their dry-run echoes, which
  # must stay in sync with reality.
  python3 - "$SCRIPTS_DIR/install.sh" << 'PY'
import re, sys
flat = re.sub(r'\s+', ' ', open(sys.argv[1]).read())
assert 'token-diet/serena:latest --context' not in flat, \
    "shell registration: image followed directly by --context (missing subcommand)"
assert 'token-diet/serena:latest", "--context' not in flat, \
    "args array: image followed directly by --context (missing subcommand)"
assert flat.count('token-diet/serena:latest serena start-mcp-server') == 4, \
    "expected 4 shell-form sites (claude+gemini real and dry-run)"
assert flat.count('"token-diet/serena:latest", "serena", "start-mcp-server"') == 2, \
    "expected 2 array-form sites (codex TOML + cowork)"
PY
}

@test "install: Install.ps1 docker-mode serena registrations supply the subcommand (static guard)" {
  # Same no-ENTRYPOINT contract as install.sh, guarded statically for the
  # Windows installer (4 array-form sites: $serenaArgsBase, codex TOML,
  # cowork, opencode/vscode). Also guards the v1.15.19 port: MCP stdio hosts
  # exec argv with no shell, so a literal `$(pwd)` mount never expands; all
  # four sites must use the relative "." mount docker resolves against its
  # own invocation cwd.
  python3 - "$SCRIPTS_DIR/Install.ps1" << 'PY'
import re, sys
flat = re.sub(r'\s+', ' ', open(sys.argv[1]).read())
assert 'token-diet/serena:latest", "--context' not in flat, \
    "image followed directly by --context (missing subcommand)"
assert flat.count('"token-diet/serena:latest", "serena", "start-mcp-server"') == 4, \
    "expected 4 array-form sites in Install.ps1"
assert '$(pwd):/workspace' not in flat, \
    "literal $(pwd) mount survives in an MCP stdio argv (never expands without a shell)"
assert flat.count('".:/workspace:ro"') == 4, \
    "expected 4 relative-mount sites in Install.ps1"
PY
}
