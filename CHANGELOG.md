...
### Fixed
* `forks/tilth/benchmark/` — hardcoded `/Users/<redacted>/.cargo/bin/tilth` and workspace results path replaced with env-var/PATH resolution and repo-local fallback
...
- 2026-07-22: fix(install): disambiguate the tilth `cargo install --git` package name. Running the real (network) installer on this machine to pick up the prior fixes surfaced a live failure: `cargo install --git <tilth-repo> --rev <sha> --force` aborted with "multiple packages with binaries found: tilth, tilth-fuzz" because the upstream tilth repo now carries a `fuzz/` package alongside the tilth binary crate, and `cargo install --git` searches the whole cloned repo for any `Cargo.toml` with a `[[bin]]` — unlike `--path`, which targets one manifest and was unaffected (the `--local` install path is fine). RTK's equivalent invocation is unaffected (its repo has no second binary package); ICM's already passed an explicit `icm-cli` package name. Both tilth call sites (the pinned `--rev` branch and the unpinned upstream-HEAD fallback) now pass `tilth` explicitly, matching ICM's pattern. Verified against the real upstream repo (`cargo install --git https://github.com/artificemachine/tilth --rev <pinned sha> tilth --force` succeeds). 278 bats / 72 pytest, 0 fail. Version 1.15.20 to 1.15.21.
- 2026-08-10: feat(tools): rtk-mcp — Python MCP server exposing rtk CLI subcommands as 71 MCP tools (rtk_ls, rtk_read, rtk_git, etc.); install.sh registers it across codex/claude/cowork/opencode/gemini via tdconfig
- 2026-08-10: fix(tools): rtk-mcp — drop functools.wraps(_run_rtk) so FastMCP exposes only the (args) parameter (was leaking _run_rtk's (cmd, args) signature → pydantic validation error)
- 2026-08-10: fix(tools): rtk-mcp uninstall symmetry — add remove_json_key/remove_opencode_mcp_key for rtk-mcp across claude/cowork/opencode/gemini; extend codex TOML uninstall regex; wrap install_rtk_mcp python heredocs in `if_` so set -e does not kill install on malformed JSON
- 2026-08-14: fix(install): Serena Docker-mode dispatch contract — idempotent launcher subcommand, OpenCode >=1.18 entry shape, and missing subcommand in docker registrations. Three related defects, surfaced live on this machine as OpenCode `serena MCP error -32000: Connection closed`. (1) The launchers written to ~/.local/bin/serena (both docker and uvx runtimes) inject `serena start-mcp-server` unconditionally, while the host MCP registrations (OpenCode writer, codex TOML) ALSO pass it explicitly; the doubled subcommand lands as a positional project argument and serena exits 2 with "--project-from-cwd cannot be used with --project or positional project argument" (observed in ~/.keylogger-mcp/proxy/serena/*.jsonl), which MCP clients surface as -32000 / connection closed. Both launcher templates now strip a caller-supplied leading `start-mcp-server` (idempotent dispatch). (2) The SERENA_KEYLOGGER_WRAPPER OpenCode writer introduced by #75 emitted {command: <string>, args: [...]}, but OpenCode >=1.18 validates mcp entries against a schema whose `command` is a single argv array with no `args` key, and silently ignores non-conforming entries ("Ignoring MCP config entry without type" in the opencode log — this is what dropped every wrapped MCP server from OpenCode on this machine the same day). Both OpenCode writers (local-mode and online) now emit the full argv, wrapper included, as the command array. (3) The docker-mode registrations for Claude Code, Codex, Cowork, and Gemini CLI let `--context=...` follow the image name directly, but docker/Dockerfile.serena intentionally carries NO ENTRYPOINT (v1.11.4 contract: the launcher/registrations are the single source of truth for the serena CLI invocation), so docker exec'd the flag as the container command and died instantly; all four sites (plus their dry-run echoes) now pass `serena start-mcp-server` after the image, and Install.ps1 receives the same fix at its four array-form sites ($serenaArgsBase, codex TOML, cowork, opencode/vscode). Six new bats regressions: static idempotence guard on both launcher templates; a functional test that generates the real --local launcher against a mocked docker and asserts the container command carries the subcommand exactly once under BOTH caller dialects (explicit subcommand and bare), plus the --local OpenCode entry shape; an OpenCode >=1.18 schema-shape test for the online writer; and static no-ENTRYPOINT-contract guards for install.sh and Install.ps1. Known residual (not fixed here, Windows-only): Install.ps1 docker mounts still use the literal `$(pwd)` form the v1.15.19 fix replaced with `.` in install.sh. Version 1.15.21 to 1.15.22.
- 2026-08-14: fix(install): port the v1.15.19 relative-mount fix to Install.ps1. The Windows installer's four docker-mode serena registrations ($serenaArgsBase, codex TOML, cowork, opencode/vscode) still emitted the literal `$(pwd):/workspace:ro` mount arg that install.sh replaced with `.` in v1.15.19: MCP stdio hosts exec the configured argv directly with no shell, so the PowerShell-escaped `$(pwd)` literal never expanded and docker received it verbatim. All four sites now use the relative `.:/workspace:ro` mount, which docker resolves against its own invocation cwd inherited from the host's spawn of the MCP subprocess (the codex TOML block in the same file already used the relative form — this converges the other three onto it). The Install.ps1 static bats guard now also asserts no `$(pwd):/workspace` literal remains and exactly four relative-mount sites exist. Version 1.15.22 to 1.15.23.
- 2026-08-17: docs(audits): add 2026-07-22 golive readiness audit report + progress log, and standalone RTK.md quick reference.
- 2026-08-17: fix(docs): redact two literal home paths in the 2026-07-22 golive audit doc — CI Path Leak Guard full-tree scan flagged them (the audit's own finding, ironically re-triggered by quoting the leaked path verbatim).
- 2026-08-17: fix(release): gate release.yml's auto-release on the tagged commit's Tests (bats + pytest) check-run actually being 'success' before creating a GitHub release. Previously the workflow only verified the pushed tag matched TD_VERSION -- it never checked whether the Tests workflow had passed on that commit, so a bare tag push could publish a release with zero test-gating (hit live this session: v1.15.23 was tagged and released without the server-side Tests check having been confirmed green first). New bats regression asserts the check-runs gate exists and runs before the release-creation step. Version 1.15.23 to 1.15.24.
- 2026-08-17: chore: no-op commit to retrigger the push webhook -- the merge commit from PR #78 (b87703f) never got a Tests workflow run (10+ min, no check-runs at all), most likely a delivery gap from the GitHub API 503 outage hit earlier this session. No workflow_dispatch trigger exists on test.yml to force it manually, so this trivial commit exists solely to produce a fresh push event for the new release-gate (PR #78) to verify against before tagging v1.15.24.
- 2026-08-19: fix(security): Serena phones home by default and token-diet's opt-out was a no-op. forks/serena/src/serena/agent.py `_send_usage_info()` GETs oraios-software.de/serena_usage.php on EVERY agent start (params: OS, version, backend, dashboard flag, context — no file or code content). It is opt-OUT, and the only env var its code reads is SERENA_USAGE_REPORTING. token-diet had been setting SERENA_NO_TELEMETRY=1 in docker/Dockerfile.serena and docker/compose.yml — that string appears nowhere in Serena's source, so it did nothing; Docker mode was protected solely by `network_mode: none` / `--network none`, and the DEFAULT uvx path (the launcher written to the user's local bin) had no protection at all and phoned home once per MCP session. Both docker artifacts now set SERENA_USAGE_REPORTING=false, and the uvx launcher template exports it before exec. Four new bats regressions (static guards on install.sh + both docker artifacts, a functional check that the generated uvx launcher carries the export, and a guard that the compliance doc no longer claims telemetry is "stripped").
- 2026-08-19: fix(compliance): correct three false claims in compliance/security-audit.md. (1) RTK telemetry was described as "stripped in fork" — it is not: forks/rtk/src/core/telemetry.rs is a live 606-line module with a real ureq::post, and `ureq` is a normal compiled dependency. It is inert in a token-diet build for a different reason: the endpoint comes from option_env!("RTK_TELEMETRY_URL") at compile time and install.sh never sets it, so the call site is unreachable dead code; TelemetryConfig also defaults to enabled=false/consent_given=None and `rtk init -g` never prompts non-interactively. (2) The Serena row claimed all http matches were install-time LSP downloads — false, see the telemetry entry above. (3) The network-isolation table claimed "no network crates in source" for RTK — false. Each corrected row now states the mechanism and is dated. The prior "stripped" wording traced back to a CHANGELOG entry describing a since-reverted upstream state.
- 2026-08-19: fix(dashboard): the alert-banner feature was completely dead. The dashboard JS reads `d.loops.loops[]` and `d.leaks.leaks[]`, but collect() never emitted either key — the producing code had been replaced at some point by a literal `alerts = []` plus the comment "(OMITTING DUPLICATE ALERT LOGIC FOR BREVITY - it was already there)", so both banners could never render. Added loops_stats()/leaks_stats() to scripts/token-diet-dashboard and wired both keys into collect(), reusing the same thresholds (loops >=3, leaks >=2) and the same "By Command" table parsing the `token-diet loops`/`token-diet leaks` CLI uses, so dashboard and CLI cannot disagree. Note the JSON formatter of `rtk gain` emits only `summary` with no `commands` key, which is why the earlier "-H --format json" approach in 1.2.1 also silently produced nothing — per-command data exists only in the text output. Also removed an unreachable duplicate cache-set/return block after collect()'s first return. Known limitation, stated honestly: rtk aggregates `rtk read` without file arguments, so the leak detector's file regex rarely matches in practice — same structural limit the CLI has. 5 new pytest regressions.
- 2026-08-19: fix(uninstall): uninstall.sh is the ROLLBACK path and had no ERR trap and no consistent JSON error handling — so one malformed host config killed it mid-run with a raw Python traceback and no record of what had already been removed. Added a TD_REMOVED/_td_on_error trap mirroring install.sh's contract (reports what was already removed, notes that re-running is safe, points at the .bak-token-diet-<ts> backups). Standardized all six embedded json.load sites on one td_load() helper: three had NO exception handling at all (remove_opencode_mcp_key, remove_json_key, remove_vscode_template_server), two swallowed every exception via bare `except Exception` (remove_opencode_plugin, remove_hook_entry), and one narrowed correctly but still failed silently (strip_opencode_rules). td_load now backs the file up to <path>.corrupt-<ts>, prints a token-diet-voiced SKIP line, and continues — deliberately NOT install.sh's exit-3 abort, because a rollback must keep cleaning the remaining hosts rather than stranding the user half-uninstalled. 3 new bats regressions. Version 1.15.24 to 1.15.25.
- 2026-08-19: fix(security): stop exempting CHANGELOG.md from path-leak-scan.sh. The full-tree guard skipped CHANGELOG.md wholesale so entries could quote a third-party path, and that exemption is exactly how a real home path reached a public file with a green CI run on 2026-08-17: the changelog entry describing a path redaction quoted the very path it was redacting, and the scanner skipped the one file it had been written into. SKIP_PATTERNS now exempts only the scanner itself and tests/path-leak.bats (both hold deliberately planted fixtures). Redacted the two real usernames this surfaced in tracked content — one pre-existing third-party path from the tilth benchmark entry, one self-inflicted on 2026-08-17 — so the tree passes the stricter scan. Committed with --no-verify under the docs-only exception, since the append-only check blocks any modification of an existing CHANGELOG line and redacting a leaked path is necessarily a modification. Note the git HISTORY still contains both strings; that needs a separate filter-repo pass with --replace-text, --replace-message AND --mailmap (verified on a throwaway clone: text-only leaves 1 commit-message hit and 2 commit-author hits behind).
- 2026-08-19: chore(forks): bump all three pinned submodules to pick up lint/test fixes landed upstream in the forks. This is the delivery step for the clippy+zig debt recorded in the 2026-07-22 audit — until the gitlinks move, token-diet still builds the old forks. rtk ba84820 to 71dc930: two `dead_code` false positives that broke `cargo clippy --all-targets -- -D warnings` are now scoped with `#[cfg_attr(test, allow(dead_code))]` (FILTERS_TOML and TomlFilterRegistry::load are live in the real binary via load() -> REGISTRY lazy_static -> find_matching_filter -> main.rs, but --all-targets also builds a test binary that never links main's call path, so the chain read as dead there and `[lints.rust] warnings = "deny"` promoted it to a hard error). tilth 0ada7c1 to e9cd882: 63 clippy errors cleared, most mechanically via `cargo clippy --fix`; the two production changes are behaviour-preserving (`find_git_root` uses `?` instead of a match arm returning None; `validate_pager` gains `#[must_use]`), and the 19 remaining `format_push_string`/`format_collect` hits were all `#[cfg(test)]` fixture builders, now allowed via `#![cfg_attr(test, allow(...))]` so production code still gets the crate's opted-in pedantic lints. That branch also carried a pre-existing `cargo fmt --check` failure on tilth's main (`src/install.rs`, left over from the TILTH_KEYLOGGER_WRAPPER change) which had been turning every tilth PR red on arrival. serena 48d5b27 to dd0ae55: the Zig suite now skips cleanly when `zig`/`zls` are absent instead of producing 12 collection ERRORs — Zig was the only toolchain-gated language missing from `_determine_disabled_languages()`, while ~12 others already route through `language_tests_enabled`. Net effect on this repo: `scripts/release.sh` now reports both Rust forks clippy-clean (verified: `cargo clippy --all-targets -- -D warnings` exits 0 for rtk and tilth at the new pins) instead of emitting 65 warnings, and a local `uv run pytest` in forks/serena no longer errors out on zig. Version 1.15.25 to 1.15.26.
- 2026-08-20: fix(repair): `token-diet repair` silently did nothing for OpenCode. Two independent bugs, both making repair unable to act on configs `doctor` correctly diagnoses. (1) `cmd_repair` resolved the OpenCode config as `~/.opencode.json` only, while `cmd_doctor` prefers the XDG path `~/.config/opencode/opencode.json` when present — which is what install.sh actually writes, so on a modern install repair never opened the right file. (2) Even pointed at the right file, `_repair_mcp_json` read only the `mcpServers` key with a string `command`, but OpenCode uses the `mcp` key with an argv ARRAY. Both read and write sides are now dialect-aware, and the array form preserves trailing argv (e.g. `["tilth", "--mcp"]`) by swapping element 0 only — rewriting it to a bare string would violate the OpenCode >=1.18 schema that requires an array. New bats regression covering an XDG-path OpenCode config end to end.
- 2026-08-20: docs(branch-protection): correct two false claims. The file opened by stating `main` has "no branch protection" and that the API returns "Branch not protected"; `main` is in fact fully protected (verified against the live API: enforce_admins true, three required contexts, strict mode, force-push blocked). The example JSON also read `"enforce_admins": false` while the prose directly below it argued `true` is the correct value — the prose had been updated when the decision changed, the JSON had not. Both corrected against the live API response, with a note recording that the doc had drifted. Version 1.15.26 to 1.15.27.
- 2026-08-20: chore(forks): bump rtk to c069e75 and tilth to 78780bc to deliver the RUSTSEC fixes. This is the step that actually clears the advisories for token-diet users — the fork PRs alone changed nothing here, since the build follows the pinned gitlinks. Clears 4 advisories total: RUSTSEC-2026-0204 (crossbeam-epoch invalid pointer dereference, both forks), plus RUSTSEC-2026-0195 (quick-xml unbounded namespace-declaration allocation, a memory-exhaustion DoS) and RUSTSEC-2026-0194 (quick-xml quadratic duplicate-attribute checking) in rtk. Verified at the new pins: `cargo audit --file forks/rtk/Cargo.lock` and the tilth equivalent both report 0 vulnerabilities, down from 3 and 1.
- 2026-08-20: fix(install): the Serena launcher write failed hard when the target path was a dangling symlink. Older installs symlinked the launcher into a venv inside the checkout; if that checkout was later moved, re-cloned or removed, the symlink dangles — and a shell redirect through a dangling symlink fails with ENOENT, so install.sh aborted at the redirect with a bare "No such file or directory" from bash, no diagnosis, and a non-zero exit after most of the stack had already installed successfully. Hit live while validating v1.15.27 on a machine whose previous checkout had been replaced. Both launcher write sites (docker runtime and uvx runtime) now clear the path before writing. New bats regression plants a genuinely dangling symlink at the launcher path and asserts install exits 0 and leaves a real executable file, not a broken link. Version 1.15.27 to 1.15.28.
