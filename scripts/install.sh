...
install_serena() {
  ...
  if $LOCAL_MODE; then
    ...
    if [ "${DRY_RUN:-false}" != "true" ]; then
      # Clear any existing entry first. Older installs symlinked this path into
      # a venv inside the checkout; if that checkout was later moved or removed
      # the symlink dangles, and `cat >` through a dangling symlink fails with
      # ENOENT — install.sh then died with a bare bash error and no diagnosis.
      rm -f "$serena_launcher"
...
# Idempotent subcommand: MCP host registrations pass an explicit
# `start-mcp-server` (see the OpenCode/Codex writers below); strip it so the
# container does not dispatch it twice. A doubled subcommand lands as a
# positional project arg and serena exits 2 ("--project-from-cwd cannot be
# used with --project or positional project argument"), which MCP clients
# surface as a -32000 connection-closed error.
...
WRAP
    ...
    if [ "${DRY_RUN:-false}" != "true" ]; then
      # See the Docker branch above: clear a possibly-dangling symlink first,
      # otherwise `cat >` fails with ENOENT on any upgrade over a venv-mode install.
      rm -f "$serena_launcher"
      cat > "$serena_launcher" <<WRAP
#!/usr/bin/env bash
# token-diet Serena launcher (uvx runtime) — generated by install.sh
# Do not edit; rerun \`bash install.sh --serena-only\` to regenerate.
# Idempotent subcommand: host MCP registrations pass an explicit
...
WRAP
  ...
  if $HAS_CLAUDE; then
    if [ "${DRY_RUN:-false}" = "true" ]; then
      $LOCAL_MODE \
        && dryrun "claude mcp add --scope user serena -- docker run ... -v .:/workspace:ro token-diet/serena:latest serena start-mcp-server --context=claude-code" \
...
def mcp_entry(name, inner_cmd):
    # OpenCode >= 1.18 validates mcp entries against a schema whose `command`
    # is a single argv ARRAY and which has no `args` key; entries shaped as
    # {command: <string>, args: [...]} fail validation and are silently
    # ignored ("Ignoring MCP config entry without type"). Keep the whole
    # argv — wrapper included — in the command array.
    if WRAPPER:
        return {
            "type": "local",
            "command": [WRAPPER, "--name", name, "--"] + inner_cmd,
            "enabled": True,
        }
...
def mcp_entry(name, inner_cmd):
    # OpenCode >= 1.18 requires `command` as a single argv array and has no
    # `args` key; the string-command + args shape is silently ignored by the
    # host ("Ignoring MCP config entry without type"). See the local-mode
    # writer above for details.
    if WRAPPER:
        return {
            "type": "local",
            "command": [WRAPPER, "--name", name, "--"] + inner_cmd,
...
PYEOF
  ...
  if $HAS_GEMINI; then
    if [ "${DRY_RUN:-false}" = "true" ]; then
      $LOCAL_MODE \
        && dryrun "gemini mcp add --scope user serena docker run ... -v .:/workspace:ro token-diet/serena:latest serena start-mcp-server" \
      ...
      gemini mcp add --scope user serena docker run --rm -i -v ".:/workspace:ro" --network none \
        token-diet/serena:latest serena start-mcp-server --context=gemini-cli --open-web-dashboard false --project /workspace \
        2>/dev/null \
